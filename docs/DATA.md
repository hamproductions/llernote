...
| `event-extra.json` | 744 | Generated by `scripts/build-event-extra.ts` from the-sorter's `data/raw/llfans-performances.json`: per-performance `performanceName` (Day.1, 昼の部…), `concertName` (名古屋公演…), `openTime`/`startTime`, `tourType` (ライブ・ファンミ, TV出演…), `audience`, `canceled`, `note` |

`hooks/useData.ts` merges `event-extra.json` into performances and derives `category: 'live' | 'online' | 'tv'` from `tourType` + `audience`. Re-run `bun scripts/build-event-extra.ts` after refreshing the-sorter's raw data (17 newest performances currently lack extra data and default to `live`).
