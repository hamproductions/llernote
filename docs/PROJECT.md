...
| `event-extra.json` | 765 | **Generated** by `scripts/build-event-extra.ts` from `data/raw/llfans-performances.json`: `concertName` ("名古屋公演"), `openTime`/`startTime`, `tourType` (ライブ・ファンミ, 外部のフェス, TV出演, リリイベ・ミニライブ, 外部イベント内のライブ, バーチャルライブ, 有観客バーチャルライブ, 収録配信), `audience`, `canceled`, `note` (156 populated — injuries, absences, broadcast notes). `performanceName` is emitted only where `performance-info.json` lacks a composed label, since extra wins the merge and the raw name is the barer form. **Merges into the existing file**: `data/raw/` is gitignored so CI's incremental fetch only re-writes a handful of performances, and a replace-everything build collapsed this file to ~21 entries, defaulting all 765 performances to `category: 'live'`. |
