...
export const encodeMyPick = (myPick: MyPick, rows: MyPickRow[], columns: MyPickColumn[]) => {
  const data: ShareData = {
    ...
    c: columns.map((col) => (col.type === 'slot' ? ['s', col.slot] : ['y', col.year, col.slot])),
...
export const decodeMyPick = (
  ...
  try {
    ...
    if (data.v !== 1 || !Array.isArray(data.r) || !Array.isArray(data.c)) return null;
    return {
      rows: data.r.map(([type, id]) => ({ type, id }) as MyPickRow),
      columns: data.c.map((col) =>
        col[0] === 's'
